...
from accounts.models import Department, DepartmentPermission, User, Organization
...
class AgreementForm(forms.ModelForm):
    ...
    parent_agreement = forms.ModelChoiceField(
        queryset=Agreement.objects.all(),
        required=False,
        widget=forms.Select(attrs={'class': 'form-control'}),
        label='Parent Agreement',
        empty_label='Select Parent Agreement (Optional)'
    )

    class Meta:
        model = Agreement
        fields = ['title', 'agreement_reference', 'agreement_type', 'department', 'party_name', 
                 'start_date', 'expiry_date', 'reminder_time', 'attachment','remarks', 'parent_agreement']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk and self.instance.party_name:
            try:
                vendor = Organization.objects.get(name=self.instance.party_name)
