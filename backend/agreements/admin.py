...
class AgreementAdmin(admin.ModelAdmin):
    list_display = (
        'agreement_id', 'title', 'agreement_reference', 
        'get_agreement_type', 'get_department_name', 'status', 
        'start_date', 'expiry_date', 'get_creator', 'get_parent_agreement',
        'get_child_agreements'  
    )
    list_filter = ('status', 'department', 'agreement_type', 'created_at')
    search_fields = ('title', 'party_name', 'creator__email', 'remarks')
    # date_hierarchy = 'created_at'
    filter_horizontal = ('assigned_users',)
    readonly_fields = ('status', 'agreement_id', 'created_at', 'updated_at', 'get_child_agreements')  
    fieldsets = (
        (None, {
        ...
        ('Relations', {
            'fields': ('parent_agreement', 'get_child_agreements'),  # Add get_child_agreements
            'classes': ('collapse',)
        }),
        ('Metadata', {
    ...
    def get_parent_agreement(self, obj):
        return f"{obj.parent_agreement.agreement_id} - {obj.parent_agreement.title}" if obj.parent_agreement else '-'
    get_parent_agreement.short_description = 'Parent Agreement'
    
    def get_child_agreements(self, obj):
        # Get all agreements where this agreement is the parent
        children = obj.child_agreements.all()
        if not children:
            return "No child"
        
        # Format all children as "ID - Title"
        return "\n".join([
            f"{child.agreement_id} - {child.title}" 
            for child in children
        ])
    get_child_agreements.short_description = 'Child Agreements'
