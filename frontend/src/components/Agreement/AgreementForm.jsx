...
export default function AgreementForm({ onSubmit, initialData }) {
  ...
  useEffect(() => {
    if (initialData) {
      const agreementTypeId = typeof initialData.agreement_type === 'object' ? 
        initialData.agreement_type?.id : 
        typeof initialData.agreement_type === 'string' ? 
          parseInt(initialData.agreement_type) : 
          initialData.agreement_type;
      
      setForm({
        title: initialData.title || '',
        agreement_reference: initialData.agreement_reference || '',
        agreement_type: agreementTypeId || '', // AgreementType ID
        ...
        original_filename: initialData.original_filename || '',
        remarks: initialData.remarks || '',
        parent_agreement: initialData.parent_agreement || '',
      });
      setIsEditing(true);
    }
  }, [initialData]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('Setting vendors:', vendorsData);
        if (!Array.isArray(vendorsData) || vendorsData.length === 0) {
          console.error('No vendors data received or empty array');
        }
        setVendors(vendorsData);
        
        // Fetch available agreements for parent selection
        const agreementsResponse = await axiosInstance.get('agreements/');
        setAvailableAgreements(agreementsResponse.data.agreements || []);
      } catch (error) {
        console.error('Error fetching data:', error);
        setErrors({ general: 'Failed to load form data. You may not be authorized to view this.' });
      } finally {
        setIsLoading(false);
      }
    };
    ...
    try {
      ...
      if (isEditing && initialData?.id) {
        ...
        if (form.attachment instanceof File) {
          ...
          payload.append('parent_agreement', form.parent_agreement || '');
          headers = {
          ...
          payload = {
            ...
            parent_agreement: form.parent_agreement || '',
          };
          headers = {
        ...
        payload.append('party_name', form.party_name);
        payload.append('remarks', form.remarks || '');
        // Always append parent_agreement, even if it's empty
        console.log('Parent Agreement value:', form.parent_agreement);
        payload.append('parent_agreement', form.parent_agreement || '');
  ...
  if (showPreview) {
    ...
    if (!Array.isArray(vendors) || vendors.length === 0) {
      console.error('Vendors array is empty or not an array when trying to show preview');
      return <div className="error-message">Error: Vendor data not available. Please try again.</div>;
    }
    return (
      <AgreementPreview
        data={{
          ...
          agreement_type_name: (() => {
            const selectedType = agreementTypes.find(type => type.id === Number(form.agreement_type));
            return selectedType ? selectedType.name : 'Not specified';
          })(),
          parent_agreement: (() => {
            if (!form.parent_agreement) return null;
            const foundParent = availableAgreements.find(a => String(a.id) === String(form.parent_agreement));
            console.log('Found parent in available agreements:', foundParent);
            return foundParent ? {
              id: foundParent.id,
              title: foundParent.title,
              agreement_id: foundParent.agreement_id
            } : form.parent_agreement;
          })(),
          department: form.department,
          party_name: vendors.find(v => v.id === parseInt(form.party_name))?.name || form.party_name,
  ...
  return (
    <form className="agreement-form" onSubmit={handlePreview} style={{maxWidth: 800, margin: '0 auto'}}>
      <h2 style={{ textAlign: 'center', marginBottom: '2rem' }}>{contextIsEditing ? 'Edit Agreement' : 'Create Agreement'}</h2>
      ...
      {/* This is for marking the Parent Agreement */}
      <div className="form-group">
        <label>Parent Agreement</label>
        <select
          name="parent_agreement"
          value={form.parent_agreement || ''}
          onChange={handleChange}
          className={errors.parent_agreement ? 'error' : ''}
        >
          <option value="">Select Parent Agreement (Optional)</option>
          {availableAgreements
            .filter(agreement => {
              // Don't show current agreement in dropdown when editing
              if (initialData?.id && agreement.id === initialData.id) {
                return false;
              }
              
              // Don't allow circular references by checking the agreement's ancestry
              const isInAncestryChain = (agreement) => {
                let current = agreement;
                const visited = new Set();
                
                while (current?.parent_agreement) {
                  ...
                  // If this agreement is the one we're editing, it would create a cycle
                  if (current.parent_agreement.id === initialData?.id) {
                    return true;
                  }
                  
                  // Move up the chain
                  current = current.parent_agreement;
                }
                return false;
              };

              // Filter out if it would create a circular reference
              if (isInAncestryChain(agreement)) {
                return false;
              }
            ...
            .map(agreement => (
              <option key={agreement.id} value={agreement.id}>
                {agreement.title} ({agreement.agreement_id})
              </option>
            ))}
        </select>
        {errors.parent_agreement && <div className="error-text">{errors.parent_agreement}</div>}
      </div>

      {/* parent agreement ends */}
